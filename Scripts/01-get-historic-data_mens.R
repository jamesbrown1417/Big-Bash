...
batting_match_data_innings_1_dnb <-
  ...
  transmute(match_id,
            ...
            DNB = TRUE) |> 
  left_join(player_data_bbl, by = c("match_id", "team")) |> 
  mutate(player_id = paste(player, match_id)) |>
  filter(player_id %notin% paste(batting_stats_bbl$player, batting_stats_bbl$match_id))
...
batting_stats_bbl <-
  bind_rows(batting_stats_bbl, batting_match_data_dnb) |> 
  arrange(match_id, innings, batting_position, player)

# Add Season---------------------------------------------------------
batting_stats_bbl <-
  batting_stats_bbl |> 
  left_join(match_data_bbl[,c("match_id", "season")], relationship = "many-to-one")
...
batting_stats_bbl <-
  batting_stats_bbl |> 
  left_join(match_data_bbl[,c("match_id", "team1", "team2")], relationship = "many-to-one") |> 
...
write_rds(venue_data, "Data/venue_data_bbl.rds")
